...
public class TestServiceImpl implements TestService {
    ...
    public List<TestBean> findByNameOrDescPage(int currPage) {
        ...
        Pageable pageable = PageRequest.of(currPage, 10);
        BoolQueryBuilder boolQueryBuilder = new BoolQueryBuilder();
        boolQueryBuilder.must(QueryBuilders.matchQuery("name", "格"));
        Page<TestBean> pagedStu = testDao.search(boolQueryBuilder, pageable);
        System.out.println("检索后的总分页数目为：" + pagedStu.getTotalPages());
        List<TestBean> stuList = pagedStu.getContent();
        for (TestBean s : stuList) {
